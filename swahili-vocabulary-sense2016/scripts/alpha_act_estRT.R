...
# load csv data as df:
dat <- read_csv('vocabulary_practice_data.csv')
...
sum <- dat %>% filter(userId == 60499 & lessonId == 'vocab3') 

ggplot(sum, aes(x = as.factor(repetition), y = alpha, color = as.factor(answer), shape = correct, group = as.factor(answer))) + geom_point(size = 2, alpha = 0.6) + geom_line(size = 1, alpha = 0.6) +  
  xlab('Repetition') + ylab('Rate of forgetting') + 
  theme_bw() +  
  theme(axis.text.x=element_text(size=10), axis.text.y=element_text(size=10), axis.title=element_text(size=15))

# save data as csv:
write_csv(dat, 'vocabulary_practice_data_alpha_act_estRT.csv')
